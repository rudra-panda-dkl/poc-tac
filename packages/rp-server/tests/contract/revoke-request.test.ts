...
// contracts/revoke-api.yaml: POST /revoke/request — happy path only (denial-mode status codes
// are covered by the User Story 2 integration tests, which exercise the same endpoint's
// underlying service directly).
describe("POST /revoke/request (contract, happy path)", () => {
  ...
  it("issues a fresh revocation challenge for an active Grant", async () => {
    ...
    const agentKeypair = await getOrCreateAgentKeypair(`revoke-request-contract-test-${crypto.randomUUID()}`);
    const identity = {
    ...
    expect(revokeRequestRes.status).toBe(200);
    const body = await revokeRequestRes.json();
